...
- [ ] Initialize Git repository and create `.gitignore` (AC: 6)
  - [ ] Run `git init` if not already initialized
  - [ ] Create `.gitignore` with entries: `node_modules/`, `dist/`, `.env`, `*.log`, `.DS_Store`
  - [ ] Commit initial `.gitignore`

- [ ] Create monorepo directory structure (AC: 1)
  - [ ] Create `packages/cli/` directory with `src/` subdirectory
  - [ ] Create `packages/parser/` directory with `src/` subdirectory
  - [ ] Create `packages/generator/` directory with `src/` subdirectory
  - [ ] Create `packages/templates/` directory with `src/` subdirectory
  - [ ] Create `examples/`, `tests/`, `docs/`, `scripts/` directories at root

- [ ] Configure pnpm workspace (AC: 2)
  - [ ] Create `pnpm-workspace.yaml` at root with `packages/*` glob pattern
  - [ ] Verify pnpm version 8.15.1 is installed (`pnpm --version`)

- [ ] Create root `package.json` with workspace configuration (AC: 3)
  - [ ] Define project name: `openapi-to-mcp`
  - [ ] Set version: `0.1.0`
  - [ ] Configure private: `true`
  - [ ] Add workspace scripts: `test`, `build`, `lint`, `format`
  - [ ] Add shared dev dependencies: TypeScript 5.3.3, ESLint 8.56.0, Prettier 3.2.4, Vitest 1.2.0
  - [ ] Use exact version pinning (no `^` or `~` prefixes)

- [ ] Configure TypeScript with strict mode and ES modules (AC: 4)
  ...
  - [ ] Create individual `tsconfig.json` in each package extending root config
  - [ ] Set `outDir: "dist"` and `rootDir: "src"` for each package

- [ ] Configure ESLint with consistent rules (AC: 5)
  - [ ] Create `.eslintrc.json` at root with TypeScript parser
  - [ ] Add recommended rules: `@typescript-eslint/recommended`, `eslint:recommended`
  - [ ] Configure ESLint to ignore `dist/`, `node_modules/`, `*.config.js`
  - [ ] Add script to `package.json`: `"lint": "eslint . --ext .ts"`

- [ ] Configure Prettier with consistent formatting (AC: 5)
  - [ ] Create `.prettierrc.json` at root with settings: `semi: true`, `singleQuote: true`, `tabWidth: 2`, `printWidth: 100`
  - [ ] Create `.prettierignore` with entries: `dist/`, `node_modules/`, `*.md`
  - [ ] Add script to `package.json`: `"format": "prettier --write ."`

- [ ] Create basic `README.md` (AC: 7)
  - [ ] Add project title: "OpenAPI-to-MCP Generator"
  - [ ] Add overview section explaining project purpose
  - [ ] Add "Development Setup" section with prerequisites (Node.js 20.11.0 LTS, pnpm 8.15.1)
  - [ ] Add installation instructions: `pnpm install`
  - [ ] Add available scripts: `pnpm test`, `pnpm build`, `pnpm lint`, `pnpm format`
  - [ ] Add monorepo structure diagram showing 4 packages

- [ ] Create placeholder `package.json` for each package (AC: 1, 8)
  - [ ] CLI package: name `@openapi-to-mcp/cli`, version `0.1.0`, type `module`
  - [ ] Parser package: name `@openapi-to-mcp/parser`, version `0.1.0`, type `module`
  - [ ] Generator package: name `@openapi-to-mcp/generator`, version `0.1.0`, type `module`
  - [ ] Templates package: name `@openapi-to-mcp/templates`, version `0.1.0`, type `module`

- [ ] Verify workspace installation (AC: 8)
  - [ ] Run `pnpm install` at root
  - [ ] Verify all workspace packages are linked
  - [ ] Confirm `node_modules/` created at root
  - [ ] Confirm shared dependencies installed

- [ ] Unit test setup validation
  - [ ] Run `pnpm lint` and verify no errors
  - [ ] Run `pnpm format --check` and verify code formatting
  - [ ] Verify TypeScript compilation: `pnpm tsc --noEmit` (expect errors for empty packages, but config should work)
